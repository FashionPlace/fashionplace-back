...
export class GraficaService {
    ...
    async findAll(): Promise<GraficaEntity[]> {
        return await this.graficaRepository.find({ relations: ["empresa", "productos"] });
    }

    async findOne(id: string): Promise<GraficaEntity> {
        const grafica: GraficaEntity = await this.graficaRepository.findOne({where: {id}, relations: ["empresa", "productos"] } );
        if (!grafica)
